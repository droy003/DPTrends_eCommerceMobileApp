...
public class AdminShippedOrderProductsActivity extends AppCompatActivity {
    ...
    private TextView shippedBy,shippedDate;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin_shipped_order_products);

        orderID = getIntent().getStringExtra("orderID");
        userPhone = getIntent().getStringExtra("phone");
        productList = findViewById(R.id.adminShippedProducts_list);
        productList.setHasFixedSize(true);
        layoutManager = new LinearLayoutManager(this);
        productList.setLayoutManager(layoutManager);

        orderNoText = findViewById(R.id.adminShippedProducts_orderNo);
        orderStatusText = findViewById(R.id.adminShippedProducts_orderStatus);
        orderDateText = findViewById(R.id.adminShippedProducts_orderDate);
        totalAmountText = findViewById(R.id.adminShippedProducts_totalAmount);
        userNameText = findViewById(R.id.adminShippedProducts_userName);
        userPhoneText = findViewById(R.id.adminShippedProducts_userPhone);
        userAddressText = findViewById(R.id.adminShippedProducts_userAddress);
        deliveryNameText = findViewById(R.id.adminShippedProducts_deliveryName);
        deliveryPhoneText = findViewById(R.id.adminShippedProducts_deliveryPhone);
        deliveryAddressText = findViewById(R.id.adminShippedProducts_deliveryAddress);
        shippedBy = findViewById(R.id.adminShippedProducts_shippedBy);
        shippedDate = findViewById(R.id.adminShippedProducts_shippedDate);

        relativeLayout2 = findViewById(R.id.adminShippedProducts_relativelayout2);
        relativeLayout4 = findViewById(R.id.adminShippedProducts_relativelayout4);
        moreLessIcon = findViewById(R.id.adminShippedProducts_moreLessIcon);
        ...
        deliveredButton = findViewById(R.id.adminShippedProducts_DeliveredButton);
        deliveredButton.setOnClickListener(v->{
            AlertDialog.Builder alert = new AlertDialog.Builder(this);
            alert.setTitle("Confirm Delivered?");
            alert.setMessage("Are you sure this Product is Delivered?");
            alert.setPositiveButton("Confirm", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {

                    deliveredProduct();
                    Toast.makeText(AdminShippedOrderProductsActivity.this, "Product Delivered Successfully", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(AdminShippedOrderProductsActivity.this, AdminShippedOrderActivity.class);
    ...
    private void deliveredProduct() {

        DatabaseReference orderRef = FirebaseDatabase.getInstance().getReference().child(DBNodes.dbOrders);
        DatabaseReference fromRef = orderRef.child(DBNodes.dbOrderShippedKey).child(orderID);
        DatabaseReference toRef = orderRef.child(DBNodes.dbOrderDeliveredKey).child(orderID);
        ...
        hashMap.put("deliveredBy",Prevalent.currentOnlineUser.getPhone());
        hashMap.put("deliveredOn",saveCurrentDate+"+"+saveCurrentTime);
        hashMap.put(DBNodes.dbUsers_status,DBNodes.dbOrderDeliveredKey);
        fromRef.updateChildren(hashMap).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
...
/*
        DatabaseReference orderProductsRef = FirebaseDatabase.getInstance().getReference().child("OrderProducts");
        DatabaseReference fromProdRef = orderProductsRef.child("OrderShipped").child(orderID).child("Products");
        DatabaseReference toProdRef = orderProductsRef.child("OrderDelivered").child(orderID).child("Products");

        fromProdRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                toProdRef.setValue(snapshot.getValue()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
 ...
 */
        ...
        orderRef.child("OrderShipped").child(orderID).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
        ...
        /*
        orderProductsRef.child("OrderShipped").child(orderID).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
        ...
        statusMap.put(DBNodes.dbUsers_status,DBNodes.dbOrderDeliveredKey);
    ...
    private void orderDetails(String orderID) {
        ...
        FirebaseDatabase.getInstance().getReference().child(DBNodes.dbOrders)
                .child(DBNodes.dbOrderShippedKey).child(orderID)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if(orderDet.getShippedOn()!=null){
                            shippedBy.setText(new StringBuilder().append("Shipped On: ").append(orderDet.getShippedOn()).toString());
                        }
                    }
